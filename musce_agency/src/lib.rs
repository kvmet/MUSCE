//! `musce_agency`: the generic mechanism for autonomous agent behavior (the
//! affordance vocabulary now; the planner and arbiter later) that a game
//! consumes. Game content, the concrete affordances and the relation/component
//! vocabulary their predicates name, lives in the consumer crate, never here.
//! See `docs/architecture/agency/`.
//!
...
impl Term {
    ...
    /// existential clause (see [`bind_var`]).
...
/// game policy over `(actor, affordance, world)`, supplied through [`CostModel`],
/// so an actor's costs can vary and later be learned (build step 6).
#[derive(Debug, Clone)]
...
/// The game-supplied reading of a predicate against the world: the read-side
/// twin of [`CostModel`]. A predicate names game vocabulary (`"contained_by"`,
/// `"armor"`) the engine never interprets, so only the game can say whether it
/// holds. The planner uses this to test whether a goal or precondition is
/// already satisfied; build step 2 uses it as the oracle that a verb's real
/// mutation matches the effect its affordance declares.
///
...
/// planner primitive (build step 3), not this trait; an implementation may treat
/// a predicate that still carries a `Var` as not held.
